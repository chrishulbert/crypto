...
const byte shift_rows_table[]     = {0,5,10,15,4,9,14,3,8,13,2,7,12,1,6,11};
const byte shift_rows_table_inv[] = {0,13,10,7,4,1,14,11,8,5,2,15,12,9,6,3};
const byte lookup_rcon[]={0x8d,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36,0x6c,0xd8,0xab,0x4d,0x9a};
...
void xor(byte *a, const byte *b, int n) {
  int i;
  for (i=0;i<n;i++)
    a[i] ^= b[i];
}
...
void xor_round_key(byte *state, const byte *keys, int round) {
...
// http://en.wikipedia.org/wiki/Advanced_Encryption_Standard
void EncryptAES(const byte *msg, const byte *key, byte *c) {
...
// http://en.wikipedia.org/wiki/Advanced_Encryption_Standard
void DecryptAES(const byte *c, const byte *key, byte *m) {
  int i; // To count the rounds
