...
class Program
{
  ...
  static byte[] Expand(byte[] key) {
    ...
    while (keys.Count<176) {  // Until we have 176 bytes of expanded key, we do the following:
  ...
  /// http://en.wikipedia.org/wiki/Advanced_Encryption_Standard
  /// </summary>
  static byte[] Encrypt(byte[] m, byte[] k) {
    // Key expansion
    byte[] keys = Expand(k);
  ...
  /// </summary>
  static byte[] Decrypt(byte[] m, byte[] k) {
    // Key expansion
    byte[] keys = Expand(k);
  ...
  static void Main(string[] args) {
    Console.WriteLine("Test AES crypto");

    byte[] key = ToBytes("12345612345612345612345612345612");
    byte[] msg = ToBytes("abcdefabcdefabcdefabcdefabcdefab");
  
    byte[] crypt = Encrypt(msg,key);
    byte[] clear = Decrypt(crypt,key);
  
    Pretty("Key", key);
    Pretty("Original message", msg);
    Pretty("Encrypted (should be 85-E5...71-02)", crypt);
    Pretty("Decrypted (should be same as original)", clear);
  }
}